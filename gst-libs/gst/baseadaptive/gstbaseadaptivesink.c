...
typedef struct
{
  ...
  gboolean before_first_fragment;
...
gst_base_adaptive_sink_pad_data_new (GstPad * pad)
{
  ...
  pad_data->before_first_fragment = TRUE;
...
gst_base_adaptive_sink_class_init (GstBaseAdaptiveSinkClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_TIME_SHIFT,
      g_param_spec_uint64 ("time-shift", "Time shift",
          "The size of the rolling window. 0 means no rolling window",
          0, G_MAXUINT64, DEFAULT_TIME_SHIFT, G_PARAM_READWRITE));
...
gst_base_adaptive_sink_init (GstBaseAdaptiveSink * sink)
{
  ...
  sink->max_window = 0;
  sink->min_cache = 2;
  sink->time_shift = DEFAULT_TIME_SHIFT;
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_TITLE:
      ...
      break;
    case PROP_TIME_SHIFT:
      sink->time_shift = g_value_get_uint64 (value);
      break;
    default:
    ...
    GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_TITLE:
      ...
      break;
    case PROP_TIME_SHIFT:
      g_value_set_uint64 (value, sink->time_shift);
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
...
gst_base_adaptive_sink_start (GstBaseAdaptiveSink * sink)
{
  ...
  sink->streams_manager->time_shift = sink->time_shift;
}
    ...
    GstPad * pad, GstClockTime ts, guint count)
{
  ...
  if (!gst_pad_push_event (pad, event)) {
    GST_DEBUG_OBJECT (sink, "Failed to push upstream force key unit event "
...
#if 0
static gboolean
do_sync (GstObject * element, GstBuffer * buffer)
{
  gboolean ret = FALSE;
    ...
    GstBuffer * buffer)
{
  ...
  if (pad_data->new_fragment) {
    ...
    pad_data->before_first_fragment = FALSE;
    pad_data->new_fragment = FALSE;
  ...
  if (pad_data->fragment == NULL) {
    /* if we are here before the first fragment is created then we are
     * recieving headers. We add them to the header list if it hasnt been
     * created via caps.
     */
    if (pad_data->streamheaders == NULL) {
      if (pad_data->before_first_fragment) {
        gst_buffer_ref (buffer);
        if (pad_data->streamheaders == NULL) {
          pad_data->streamheaders = buffer;
        } else {
          pad_data->streamheaders =
              gst_buffer_append (pad_data->streamheaders, buffer);
        }
      } else
        GST_DEBUG_OBJECT (sink,
  ...
  if (sink->needs_init_segment && !sink->prepend_headers) {
    if (GST_BUFFER_FLAG_IS_SET (buffer, GST_BUFFER_FLAG_HEADER)) {
      GST_ERROR_OBJECT (sink, "Dropping headers buffer");
      goto quit;
    }
  }
#endif
...
static void
delete_file (gchar * fileloc, GstBaseAdaptiveSink * sink)
{
  gchar *path = g_build_filename (sink->output_directory, fileloc, NULL);
  g_remove (path);
  g_free (path);
}
    ...
    GstBaseAdaptivePadData * pad_data, GstClockTime stop_ts)
{
  ...
  if (sink->delete_old_files && old_files != NULL) {
    g_list_foreach (old_files, (GFunc) delete_file, sink);
  }
  g_list_foreach (old_files, (GFunc) g_free, NULL);
