...
#define MPD_DATETIME_FORMAT "%Y-%m-%dT%T%z"
...
gboolean
gst_media_presentation_write_current_datetime_attribute (xmlTextWriterPtr
    writer, const gchar * name)
{
  gboolean ret;
  time_t now;
  struct tm *t;
  ...
  now = time (NULL);
  t = localtime (&now);
  if (strftime (buf, sizeof (buf), MPD_DATETIME_FORMAT, t) == 0)
    return FALSE;
  ret = gst_media_presentation_write_attribute (writer, name, buf);
  return ret;
}
