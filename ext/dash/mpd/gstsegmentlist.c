    ...
    GstMediaSegment * segment, GList ** removed_fragments)
{
  ...
  if (G_UNLIKELY (g_list_length (info->segments) == 0)) {
    ...
    if (info->time_shift > 0) {
      guint64 oldest_time_shift_point, old_frag_end;
      GstMediaSegment *seg;
      GList *link = NULL;
      if (segment->start_ts > info->time_shift) {
        oldest_time_shift_point = segment->start_ts - info->time_shift;
        seg = (GstMediaSegment *) info->segments->data;
        old_frag_end = seg->start_ts + seg->duration;
        while (old_frag_end < oldest_time_shift_point) {
          link = info->segments;
          info->segments = g_list_remove_link (info->segments, link);
          seg = (GstMediaSegment *) link->data;
          *removed_fragments = g_list_append (*removed_fragments,
              g_strdup (seg->url));
          gst_media_segment_free (seg);
          g_list_free (link);
          seg = (GstMediaSegment *) info->segments->data;
          old_frag_end = seg->start_ts + seg->duration;
        }
...
gst_segment_list_get_average_bitrate (GstSegmentList * info)
{
  ...
  return GST_ROUND_UP_64 ((size * GST_SECOND) / (info->duration));
}
