...
gst_adaptation_set_add_media_segment (GstAdaptationSet * adaptation_set,
    gchar * id, GstMediaSegment * segment, GList ** removed_fragments)
{
  GstRepresentation *rep;
  ...
  gst_representation_add_media_segment (rep, segment, removed_fragments);
    ...
    xmlTextWriterPtr writer)
{
  ...
  if (!gst_media_common_render (&adaptation_set->common, writer))
    return FALSE;
  if (adaptation_set->minBandwidth &&
      !gst_media_presentation_write_uint32_attribute (writer, "minBandwidth",
          adaptation_set->minBandwidth))
    return FALSE;
  if (adaptation_set->maxBandwidth &&
      !gst_media_presentation_write_uint32_attribute (writer, "maxBandwidth",
          adaptation_set->maxBandwidth))
    return FALSE;

  if (adaptation_set->common.stream_type == STREAM_TYPE_VIDEO) {
