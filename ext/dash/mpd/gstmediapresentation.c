...
gst_media_presentation_add_stream (GstMediaPresentation * mpd, StreamType type,
    ...
    guint fragment_duration, guint64 time_shift, const gchar * title)
{
  GstPeriod *active_period;
  GstRepresentation *rep;
  gchar *mpd_mime_type;
  mpd->time_shift = time_shift;
  ...
  rep =
      ...
      fragment_duration, time_shift, title);
...
gst_media_presentation_add_media_segment (GstMediaPresentation * mpd,
    ...
    guint64 offset, guint64 length, GList ** removed_fragments)
{
  GstPeriod *active_period;
  ...
  ret =
      gst_period_add_media_segment (active_period, id, segment,
      removed_fragments);
    ...
    gchar * url, guint64 offset, guint64 length)
{
  ...
  mpd->availstarttime = gst_date_time_to_iso8601_string (dt);
...
gst_media_presentation_render (GstMediaPresentation * mpd)
{
...
#if 0
  if (!gst_media_presentation_write_time_seconds_attribute (writer,
          "minimumUpdatePeriod", mpd->minimumUpdatePeriodMPD))
    goto error;
#else
  if (!gst_media_presentation_write_string_attribute (writer,
   ...
   */
#if 0
  if (!gst_media_presentation_write_current_datetime_attribute (writer,
          "availabilityStartTime"))
    goto error;
#else
  if (!gst_media_presentation_write_string_attribute (writer,
          "availabilityStartTime", mpd->availstarttime))
    goto error;
#endif
  ...
  if (mpd->time_shift) {
    if (!gst_media_presentation_write_time_seconds_attribute (writer,
            "timeShiftBufferDepth", (mpd->time_shift)))
      goto error;
  }
