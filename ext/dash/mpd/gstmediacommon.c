...
gst_media_common_render (GstMediaCommon * common, xmlTextWriterPtr writer)
{
  if (!gst_media_presentation_write_string_attribute (writer, "mimeType",
    ...
    return FALSE;
  if (common->width
      && !gst_media_presentation_write_uint32_attribute (writer, "width",
          common->width))
    return FALSE;
  if (common->height
      && !gst_media_presentation_write_uint32_attribute (writer, "height",
          common->height))
    return FALSE;
   ...
   */
  if (common->group
      && !gst_media_presentation_write_uint32_attribute (writer, "group",
          common->group))
    return FALSE;
